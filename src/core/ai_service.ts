...
- entities: Extract all referenced PERSON, LOCATION, EVENT, OBJECT, TIME, EMOTION.
  ...
  * LOCATION: Extract the single most specific physical place where the events took place. Crucially, under 'entities', return AT MOST one LOCATION entity unless the events literally happened in multiple entirely disconnected places in this fragment (e.g., 'fui al parque y luego a la biblioteca'). If a specific building, landmark, or establishment is mentioned alongside its city, state, or country (e.g., 'hospital en Armenia, Quindío', 'restaurante El Molino en Bogotá'), do NOT extract the city, state, or country as separate LOCATION entities at all. Extract ONLY the most specific place including its location qualifiers as a single entity (e.g., 'Hospital en Armenia, Quindío' or 'Restaurante El Molino en Bogotá'), and set the parent name (e.g., 'Armenia') only in the 'parent_name' property of that single entity. NEVER extract parent cities, states, or countries as separate entities if they are just defining the location of a specific place. Crucially, any physical space, structure, building, establishment, room, landmark, city, country, or natural feature where events occur is a LOCATION. NEVER classify buildings, physical spaces, or establishments as OBJECT. Use CONTEXT_LOCATION to help resolve relative references (e.g., "allí", "en ese lugar") if needed.
...
3. Keep the core description of each segment as it appears in the original text, but CRUCIALLY: clean up and remove any leading or trailing narrative connectors, relative transitions, or references that link back to entities in preceding segments (e.g., if segment 1 describes visiting 'el hospital de Armenia' and segment 2 is 'Después de salir de ese hospital, decidimos ir a comer helado', you must clean segment 2's text to 'Decidimos ir a comer helado', removing the introductory clause 'Después de salir de ese hospital' completely, while setting 'hospital de Armenia' as the inherited_location for segment 2). This keeps each segment's text clean, self-contained, and free of clutter from the previous segment's entities.
