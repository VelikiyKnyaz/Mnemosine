...
- entities: Extract all referenced PERSON, LOCATION, EVENT, OBJECT, TIME, EMOTION.
  ...
  * LOCATION: Extract the specific physical places where the events took place. Crucially, under 'entities', return AT MOST one LOCATION entity unless the events literally happened in multiple entirely disconnected places in this fragment (e.g., 'fui al parque y luego a la biblioteca'). If a specific building, landmark, park, or establishment is mentioned alongside its city, state, or country (e.g., 'hospital en Armenia, Quindío', 'iglesia en Manizales', 'restaurante El Molino en Bogotá'), do NOT extract the city, state, or country as separate LOCATION entities at all. Extract ONLY the specific place as a single LOCATION entity (e.g., 'hospital', 'iglesia del parque principal', 'Restaurante El Molino'), and specify the city or country name (e.g., 'Armenia', 'Manizales', 'Bogotá') strictly in its 'parent_name' property. NEVER extract parent cities, states, or countries as separate entities if they are just defining the location of a specific place. Crucially, any physical space, structure, building, establishment, room, landmark, city, country, or natural feature where events occur is a LOCATION. NEVER classify buildings, physical spaces, or establishments as OBJECT. Use CONTEXT_LOCATION to help resolve relative references (e.g., "allí", "en ese lugar") if needed.
...
3. Keep the core description of each segment, but CRUCIALLY: clean up and remove any leading or trailing narrative connectors, relative transitions, or references that link back to entities in preceding segments (e.g., phrases like 'Después de ir a Manizales el día anterior, ', 'Como te contaba, ', 'Luego de lo del hospital, ', 'Al día siguiente partimos hacia ', 'Después de Manizales fuimos al '). Replace or clean them so that the segment's text is self-contained and clean of prior segment entity clutter (e.g., 'Al día siguiente partimos hacia el parque del café' -> 'Partimos hacia el parque del café' or 'Estuvimos en el parque del café').
4. For each segment, identify if it should inherit any temporal (time) or spatial (location/place) context from preceding segments because this segment doesn't explicitly mention it but it is still in the same context/timeline (e.g., if segment 1 mentions 'junio del 2016' and segment 2 or 3 are part of that same trip/timeline, they should inherit the time).
   - For 'inherited_time': You must output a structured time marker string in one of the formats: 'exact_year:YYYY', 'exact_month:YYYY-MM', 'exact_date:YYYY-MM-DD', 'exact_age:N', 'age_range:N-M', 'relative_years:-N', 'life_stage:childhood|teenage|adulthood', or 'fuzzy:TEXT'. Example: if it inherits 'junio del 2016', output 'exact_month:2016-06'. Use null if no temporal context is inherited.
   - For 'inherited_location': Output the name of the place/city/landmark that is inherited from preceding segments (e.g., if the preceding segment took place in 'Manizales' and this segment is in the same place and has no new location, set it to 'Manizales'). Use null if none.
