  ...
  try {
    ...
    for (const memory of pendingMemories) {
      ...
      try {
        ...
        console.log(`AI response for ${memory.id}:`, JSON.stringify({ time_markers: aiData.time_markers, entities: aiData.entities?.map(e => e.name), ambiguities: aiData.ambiguities }));
